...
async function loadInitialData(sClass) {
    ...
    try {
        ...
        // Dodane logi do sprawdzenia wartości _NETWORK_ID i sClass
        console.log("Network ID:", _NETWORK_ID);
        console.log("sClass:", sClass);

        // Sprawdzenie, czy adres jest zdefiniowany
        if (!SELECT_CONTRACT[_NETWORK_ID].STACKING[sClass].address) {
            console.error("Address is undefined for", _NETWORK_ID, sClass);
            return; // Przerwanie wykonania funkcji, jeśli adres jest niezdefiniowany
        }
...
async function unstackTokenMain(_amount_wei, oContractStacking, sClass) {
    ...
    oContractStacking.methods
        ...
        .on("receipt", (receipt) => {
            ...
            window.location.href = "http://127.0.0.01:5500/analytic.html";
        })
        .on("transactionHash", (hash) => {
            console.log("Transaction Hash: ", hash);
        })
